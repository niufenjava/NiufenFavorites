...
public class NotebookService {
    ...
    public void noteSort(NoteSortForm sortForm){
        Long patientId = sortForm.getParentId();
        List<Long> childrenIds = sortForm.getChildrenIds();
        if(!CollectionUtils.isEmpty(childrenIds)){
            for (int i = 0; i < childrenIds.size(); i++) {
                NoteEntity sortEntity = noteService.getById(childrenIds.get(i));
                sortEntity.setParentId(patientId);
                sortEntity.setSn(i+1);
                noteService.saveOrUpdate(sortEntity);
            }
        }
    ...
    public void noteDelete(NoteDeleteForm deleteForm){
        ...
        NoteEntity entity = noteService.getById(deleteForm.getId());
    ...
    public Long noteCreate(NoteCreateForm noteCreateForm){
        ...
        List<NoteEntity> children = noteService.listByParentId(noteCreateForm.getPatientId());
        if(!CollectionUtils.isEmpty(children)){
            for (int i = 0; i < children.size(); i++) {
                NoteEntity sortEntity = children.get(i);
                sortEntity.setSn(i+1);
                noteService.saveOrUpdate(sortEntity);
            }
        }
    ...
    public List<NoteBO> noteTree(){
        ...
        Collections.sort(rootNoteChildren);
